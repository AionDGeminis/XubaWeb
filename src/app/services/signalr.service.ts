...
export class SignalRService {
  ...
  //   }
  // }
  public async leaveSubasta(idSubasta: string): Promise<void> {
    const connection = this.connection;

    // Suelta la referencia de inmediato: si connectToSubasta crea una
    // conexión nueva mientras este leave está en curso, el stop() de abajo
    // ya no puede detenerla.
    ...
    try {
      if (connection.state === signalR.HubConnectionState.Connected) {
        await connection.invoke('removeGroup', idSubasta);
        console.log(`Saliste del grupo ${idSubasta}`);
      }
    } catch (err) {
      // Si la conexión se cerró a mitad del invoke, es inofensivo: iba a morir igual.
      console.warn('removeGroup no se completó:', err);
    }

    try {
      await connection.stop();
    } catch (err) {
      console.warn('Error al detener la conexión anterior:', err);
    }
  }
