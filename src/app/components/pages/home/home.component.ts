...
import { XpressAuctionsComponent } from './components/xpress-auctions/xpress-auctions.component';
...
export class HomeComponent implements OnInit {
  ...
  checkNavigation() {
    const pages = localStorage.getItem('backTo');
    if (pages) {
      localStorage.removeItem('backTo');
    }
    // let _pages: any[] = [{ url: 'home', param: null }];
    // localStorage.setItem('backTo', JSON.stringify(_pages));
  }
    ...
    this.subastaService.ConsultarSubastaOfertarId(id).subscribe({
      next: (subasta) => {
        ...
        if (_tiempoRestante > 0) {
          // let currentRoute = {url:'home', param: null}
          // localStorage.setItem('backTo',JSON.stringify(currentRoute));
